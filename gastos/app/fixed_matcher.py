...
def find_fixed_expense_matches(concept: str, fixed_expenses, currency: str) -> list:
    ...
    return [fe for fe in fixed_expenses
            if _field(fe, "currency") == currency and words & concept_words(fe["concept"])]
...
def find_candidate_expenses(fixed_expense, expenses, limit: int = 5) -> list:
    ...
    scored = []
    for exp in expenses:
        if _field(exp, "currency") != _field(fixed_expense, "currency"):
            continue
        score = 0.0
