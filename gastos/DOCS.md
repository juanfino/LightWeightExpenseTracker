...
La moneda por defecto inicial es **ARS**. El owner puede cambiarla desde **Familia**; si no indicás otra, texto, voz y modo conversacional usan la seleccionada. Para elegir otra escribí su código o nombre, por ejemplo `Netflix 15 USD`, `Hotel 200 EUR` o `Taxi 80 reales`; el marcador se quita del concepto. `$` y “pesos” significan la moneda por defecto de la familia. Si escribís un código desconocido después del monto, el bot avisa para que lo corrijas. Los formularios aceptan ARS, USD, BRL y EUR y los totales nunca mezclan monedas. Cambiar el default no convierte nada histórico: gastos, ingresos, fijos y resúmenes conservan para siempre la moneda con la que fueron registrados.
...
En **Fijos** se configuran gastos que se repiten, como alquiler, internet o cuotas. En **Resúmenes**, cada generación usa IA sobre los datos del grupo, consume una de las 15 generaciones mensuales disponibles y puede tardar cerca de un minuto. **Preferencias del relato** permite que cualquier integrante elija énfasis, tono, extensión, un foco libre y si admite sugerencias; es una configuración compartida y se aplica recién al generar o regenerar, sin modificar el resumen que ya está en pantalla. Sólo cambia el texto narrativo: totales, gráficos y tarjetas siguen saliendo directamente de los datos. La moneda predeterminada ocupa el bloque principal; cada otra moneda usada ese mes aparece en un panel desplegable con su propio total, tres clases de gasto, forecast y quién registró, sin mezclarse con ninguna otra. Cada moneda extranjera muestra, cuando existe, un equivalente aproximado en la principal a partir de cambios propios de la familia; si nunca hubo un cambio para ese par lo dice. Es sólo para dimensionar y ningún cálculo de la página lo usa.

Cada resumen nuevo agrega **Qué podría venir el mes siguiente**. La regla siempre aparece escrita en la tarjeta: el objetivo es el mes inmediatamente posterior al resumido y se usan datos solamente hasta el mes resumido. Por ejemplo, el resumen de junio proyecta julio aunque lo abras en octubre. Cada moneda del informe se calcula por separado, con un rango total formado por gastos fijos, variables habituales y una cola para imprevistos; nunca se suman monedas. Si todavía no hay tres meses con movimientos en una moneda, se muestra la parte fija conocida y se promete completar la parte variable cuando haya historial suficiente. Cuando el mes proyectado ya tiene gastos registrados, la misma tarjeta compara el rango guardado con el total real sin reescribir la proyección original. En **Preferencias del relato** se puede pedir que la narrativa dé énfasis a esta proyección; los números siempre los calcula la app, no la IA.

Desde **Administrar familia** en el menú del avatar se abre **Familia** (`/familia`). El owner puede generar un enlace para copiar y enviar por WhatsApp; vence a los 7 días y funciona una sola vez. Quien lo recibe se une con Google o código por email como miembro. El owner también puede renombrar la familia, elegir la moneda predeterminada, remover miembros sin borrar sus gastos, transferir la propiedad y eliminar definitivamente la familia escribiendo su nombre exacto. Un miembro puede salir por su cuenta; sus gastos anteriores quedan visibles en la familia.
