...
**DB schema:** platform tables `families`, `users`, `memberships`, `sessions`, `otp_codes`, `oauth_identities`, `invitations`, `telegram_link_tokens`; tenant tables `categories`, `subcategories`, `keywords`, `expenses`, `fixed_expenses`, `income_categories`, `incomes`, `shopping_items`, `cambios_dolar`, `ipc_series`, `reports`, `expense_classifications`, `llm_calls`. Membership removals are logical (`active=false`) because expenses and incomes retain composite references to the historical membership; a partial unique index permits only one active family per user. Telegram linking stores only SHA-256 token hashes. Tenant tables carry `family_id NOT NULL` with forced RLS and composite foreign keys preventing cross-family references.
