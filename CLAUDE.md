...
- `report_preferences.py` — Canonical defaults and validation for per-family narrative preferences. The missing-row defaults compile to the previous narration prompt byte-for-byte; emphasis choices map only to fields actually present in the dossier, and the bounded focus text is treated as untrusted prompt data.
