...
- `dashboard.py` — Flask app. Timestamps stored as UTC in DB; `dashboard.py` converts to Buenos Aires time (UTC-3) for display. The dashboard-only quote uses the authenticated family's IANA timezone. It also owns the shared accounting-period boundary for Dashboard/Movimientos/Ingresos/Fijos/Dólares/Resúmenes: canonical `?period=YYYY-MM`, explicit URL over a dedicated unsigned cookie, then family-local current month; Movimientos' `year`/`month` remain separate local filters.
...
- `forecast.py` — Deterministic next-month forecast persisted with each report. The target is always the month immediately after the report period and inputs stop at that period. Per currency it combines active fixed definitions, habitual-category median/IQR estimates, and a historical tail/IQR; below three months it exposes fixed only. ARS inflation factors use only IPC observations through the cutoff and project one month; USD is nominal. Stored forecasts are never recomputed, while target-month actuals are attached read-only as a backtest when they exist.
