...
- **Version:** 7.12.0 (canonical source: `gastos/config.yaml`)
...
- **`report.py`** orchestrates dossier → classify → analyze → persist, stores `report_ai.prompt_version()` (a deterministic fingerprint of the live prompt/call configuration; legacy rows keep labels such as `"3"` untouched), and computes the separate data fingerprint (SHA256 of the period's *local* facts only — its expenses' id/amount/category/subcategory/user/date/fixed-link and dollar operations — deliberately excluding derived values like averages, so re-fingerprinting an unchanged period always reproduces the same hash even months later). The data fingerprint isn't consumed yet — it's computed now so a drift badge landing in a follow-up PR has a baseline for every report generated from 2.3.0 on. `_build_partitions()` (renamed from `_build_partition` in 7.7.0) aggregates the classification call's per-expense labels into one recurring/exceptional split per currency, never mixing an ARS expense into the USD partition or vice versa.
