...
- **Version:** 7.17.3 (canonical source: `gastos/config.yaml`)
...
**Period summaries use the same N-currency rule (7.17.3).** Ingresos, Telegram `/gastos`, the Dashboard income widget and its secondary expense totals derive their displayed currencies from activity in that period plus `families.default_currency`, ordered with the default first. `db.period_currency_order()` is the shared server helper and `MoneyFormat.periodCurrencyOrder()` is its browser counterpart; neither path adds amounts from different currencies.
