...
- **Version:** 7.13.0 (canonical source: `gastos/config.yaml`)
...
**Database:** PostgreSQL 17 with Alembic (current head: `0011`). Platform tables are `families`, `users`, `memberships`, `sessions`, `otp_codes`, `oauth_identities`, `invitations`, `telegram_link_tokens` and global `infrastructure_cost_settings`/`quotes`; `quotes` is curated installation-level content with no `family_id`, tenant RLS or CRUD UI. Removing a member keeps an inactive historical membership, while a partial unique index permits only one active family per user. `memberships.onboarding_dismissed_at` (migration `0009`) lets a member manually hide the dashboard's onboarding checklist before completing it; scoped to the membership, not the user, so a new family starts fresh. Tenant tables are `categories`, `subcategories`, `keywords`, `expenses`, `fixed_expenses`, `income_categories`, `incomes`, `shopping_items`, `cambios_dolar`, `ipc_series`, `reports`, `expense_classifications`, `family_report_preferences`, `llm_calls`, `family_quota_overrides` and `system_errors`. Tenant rows carry `family_id NOT NULL` with forced RLS on transaction-local `app.family_id`; composite tenant foreign keys reject cross-family references. Normal and read-only roles remain subject to RLS; `gastos_superadmin` is the dedicated `BYPASSRLS` role and is the only role used by `/superadmin` for cross-family reads. Amounts retain native ARS/USD and timestamps are UTC; families store their display timezone.

**Money arithmetic:** psycopg `NUMERIC` values stay as Python `Decimal` throughout application arithmetic. `money.py` centralizes two-place `ROUND_HALF_UP` quantization and the explicit conversion to JSON numbers at HTTP, report-persistence and LLM boundaries. Percentages, ratios and confidence remain deliberate non-money statistics; `ipc_series.value` retains its full `NUMERIC(20,12)` precision.
...
- **Persistence is append-only.** `reports` never gets an UPDATE — every generation or regeneration is a new row; the latest is displayed and the full history stays queryable. `preferences_json` preserves how that narrative was configured even after the shared `family_report_preferences` row changes. Existing rows keep `NULL` and render unchanged. `expense_classifications` remain tied to the producing report.
...
| `/ingresos?period=YYYY-MM` | `incomes.html` | Tenant-scoped income history and CRUD for the shared period in ARS/USD, with a separate fully administrable income taxonomy; members may mutate only their own rows |
...
| `/dolares?period=YYYY-MM` | `dolares.html` | USD/ARS operations: selected-period monthly summary, unfiltered history and historical-rate charts, add/edit/delete an operation — "+ Agregar cambio" opens a buy/sell toggle that relabels the amount/rate/ARS fields per direction |
| `/resumenes?period=YYYY-MM` | `resumenes.html` | Monthly AI-generated report (2.3.0; ARS/USD currency parity in 7.7.0) for the shared period — see dedicated section above. `/resumenes/YYYY-MM` redirects here for compatibility; Generate button when a period has none; understated Regenerate always available |
...
- `money.py` — central monetary boundary: parses/quantizes amounts to two decimal places with `ROUND_HALF_UP`, rounds Decimal-derived statistics explicitly, and converts Decimal-bearing structures to JSON numbers only when data leaves Python
...
- `report_preferences.py` — canonical defaults, strict web validation and tolerant storage resolution for the family's shared narrative settings. Available emphasis keys correspond only to structures present in `dossier.py`; the default resolves to the pre-7.13.0 narration behavior.
